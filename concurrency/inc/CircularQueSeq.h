...
public:
    ...
    ~CircularQueSeq()
    {
        ...
        do
        {
            use_expected = false;
            use_desired = true;
            /* 当 _atomic_using 等于 use_expected， 会把 use_desired 赋值给 _atomic_using 并返回 true, 如果不等，会把 _atomic_using 赋值给 use_desired 并返回 false */
    ...
    bool emplace(Args &&...args)
    {
        ...
        bool use_expected = false;
        bool use_desired = true;
        do /* do_while 相当于一把锁，自旋锁的思想；这个效率还不如锁 */
        {
            use_expected = false;
            use_desired = true;
        } while (!_atomic_using.compare_exchange_strong(use_expected, use_desired));
